...
_Bool compare_mask_reduce_x64_AVX2(FILE *f_out, parse_job *pj) {
    // comparison vectors
    __m256i v_delim = _mm256_set1_epi8((char)pj->pj_delim_char);
    __m256i v_linefeed = _mm256_set1_epi8((char)linefeed_char);
    __m256i v_quote = _mm256_set1_epi8((char)quote_char);
    ...
    for (size_t i = 0; i < pj->pj_buf_len; i += 64) {
        grp_0 = _mm256_loadu_epi8(pj->pj_buf + i);
        grp_1 = _mm256_loadu_epi8(pj->pj_buf + i + 32);

        // compare_mask_reduce DELIMITER and LINEFEED        
        res_0 = _mm256_cmpeq_epi8(grp_0, v_delim);
        res_1 = _mm256_cmpeq_epi8(grp_0, v_linefeed);
        res_2 = _mm256_cmpeq_epi8(grp_1, v_delim);
        res_3 = _mm256_cmpeq_epi8(grp_1, v_linefeed);
        res_0 = _mm256_or_si256(res_0, res_1);
        res_2 = _mm256_or_si256(res_2, res_3);

        m_linefeed_and_delim |= (uint64_t)_mm256_movemask_epi8(res_0);
        m_linefeed_and_delim |= ((uint64_t)_mm256_movemask_epi8(res_2) << 32);

        // compare_mask_reduce QUOTE
        res_0 = _mm256_cmpeq_epi8(grp_0, v_quote);
        res_1 = _mm256_cmpeq_epi8(grp_1, v_quote);
        m_quote |= (uint64_t)_mm256_movemask_epi8(res_0);
        m_quote |= ((uint64_t)_mm256_movemask_epi8(res_1) << 32);
